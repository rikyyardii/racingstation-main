...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    <div class="tutorial-content">
      ...
      <div class="troubleshoot-box">
        <p>\u23F1\uFE0F <strong>Firstly, follow our Twitter for the real-time RacingStation Stream live streaming schedule information.</strong></p>
      </div>
  ...
  `
    ...
    const replaceTwitterLinks = () => {
      if (!richContent.value || !twitterLink.value)
        return;
      const twitterTexts = ["follow our Twitter", "Follow our Twitter", "follow our Twitter account", "Follow our Twitter account"];
      const findAndReplace = (node) => {
        if (node.nodeType === Node.TEXT_NODE) {
          const text = node.nodeValue;
          let newHtml = text;
          twitterTexts.forEach((twitterText) => {
            if (text.includes(twitterText)) {
              const regex = new RegExp(twitterText, "gi");
              newHtml = newHtml.replace(regex, `<a href="${twitterLink.value}" target="_blank" rel="noopener noreferrer" class="twitter-link">${twitterText}</a>`);
            }
          });
          if (newHtml !== text) {
      ...
      Array.from(richContent.value.childNodes).forEach(findAndReplace);
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<span>${ssrInterpolate(tutorialData.value.reading_time)} min read</span></div></div><div class="rich-content">${(_a = tutorialData.value.content) != null ? _a : ""}</div></div></article><div class="adsterra-container"></div></main>`);
...
//# sourceMappingURL=tutorial-BOMq_Con.mjs.map
