  ...
  `
    ...
    const processedContent = computed(() => {
      ...
      tutorialImages.value.forEach((image) => {
        ...
        const imageHtml = `
      <div class="tutorial-image" data-image-id="${image.id}">
        <figure>
          <img src="${image.src}" 
               alt="${image.alt}" 
               loading="lazy"
  ...
  `;
    ...
    const seoDescription = computed(() => {
      if (!tutorialData.value.content)
        return "Complete Racing Tutorial Guide - Learn everything about racing techniques and strategies";
...
//# sourceMappingURL=tutorial-CHI4IkuW.mjs.map
